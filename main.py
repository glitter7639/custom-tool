...
# --- 5. 計算ロジック（天井逆算・実効確率方式） ---

# ① 天井までの物理的な残り
limit_rem_g = max(1, target_g - current_g)

# ② 実効初当たり確率（天井を考慮した段階的ブースト）
base_prob = 1 / conf_hit

if limit_rem_g > 600:
    effective_prob = base_prob * 0.9
elif limit_rem_g > 400:
    effective_prob = base_prob * 1.1
elif limit_rem_g > 200:
    effective_prob = base_prob * 1.5
elif limit_rem_g > 100:
    effective_prob = base_prob * 2.5
else:
    effective_prob = base_prob * 5.0
